...
jqueryWidget: {
    _init: function () {
        ...
        this.prompt = dget(this.options, "prompt", "");
        ...
        $('.large').prepend($("<center>").append($("<p>").html('<br/><b>' + this.prompt + '</b><br/>')));
        $('.large').append($('<br>'));
        $('.large').append($("<center>").append($('<p>').html("Which one of these is the most likely in the world?")));
            
        if (this.continueMessage) {
            $('.large').append($('<br>'));
            $('.large').append($("<center>").append($('<p style="font-size: 24px;">').append($("<a>").attr('href', '').text("\u2192 " + this.continueMessage)
